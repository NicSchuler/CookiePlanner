...
load_input_csvs <- function(ingredients_csv, recipes_csv){
  # read the ingredients table
  ingredients_tbl <- read.csv2(file=ingredients_csv, encoding = "UTF-8") %>%
    rename(Number=1) %>% 
    select(Number, Ingredient, Measure, App_Qty_Input, Ingr_full_name) %>%
  ...
  # read the recipes table
  recipes_tbl <- read.csv2(file=recipes_csv, encoding = "UTF-8", col.names = c("Number", "Recipe", "Comment", all_of(ingr), paste("X", c(1:(30-length(ingr))), sep="")), check.names=FALSE) %>%
    select(c("Number", "Recipe", "Comment", all_of(ingr))) %>%
